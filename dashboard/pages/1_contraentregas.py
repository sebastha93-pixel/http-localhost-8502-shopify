...
st.set_page_config(
    page_title="MALE'DENIM · Contraentregas",
    page_icon="💰",
    layout="wide",
...
# ── Sidebar ───────────────────────────────────────────────────────────────────
ruta_csv, ts, filtro_nivel, filtro_zona = render_sidebar("Contraentregas COD")
...
with col_h:
    st.markdown(f"""
        ...
        <p class="subtitulo">Pedidos con recaudo pendiente · MALE'DENIM</p>
...
valor_en_riesgo = df[df["Nivel"].isin(["CRITICO","RIESGO"])]["Valor COD"].apply(_parse_cod).sum()
valor_total_cod = df_base_cod["Valor COD"].apply(_parse_cod).sum()
...
with k4:
    st.markdown(f"""<div class="kpi-card kpi-extra">
        <p class="kpi-num" style="font-size:1.3rem;">${valor_en_riesgo:,.0f}</p>
...
with k5:
    st.markdown(f"""<div class="kpi-card" style="background:{DEEP_INK};border-left:4px solid {STEEL_BLUE};">
        <p class="kpi-num" style="font-size:1.3rem;">${valor_total_cod:,.0f}</p>
        <p class="kpi-label">Total portafolio COD</p>
        <p class="kpi-sub">{total_cod} pedidos activos</p></div>""", unsafe_allow_html=True)
...
with cg1:
    st.markdown("<div class='sec-title'>Distribución por zona — COD</div>", unsafe_allow_html=True)
    if not df.empty:
...
with cg2:
    st.markdown("<div class='sec-title'>Días en tránsito — excepciones COD</div>", unsafe_allow_html=True)
...
COLS = ["Prioridad","Nivel","Score","Orden","Cliente","Teléfono",
        "Ciudad","Zona","Días","Días sobre SLA","Valor COD","Transportadora","Novedad","Motivo riesgo"]
