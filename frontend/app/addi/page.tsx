        ...
        <Card>
          <CardContent className="p-8">
            <div className="flex items-start gap-3 mb-4">
              <AlertTriangle className="h-6 w-6 text-rust flex-none mt-0.5" />
              <div>
                <p className="font-semibold text-ink">No se pudo conectar con Addi</p>
                <p className="text-sm text-graphite mt-1">{statusQ.data?.error || "Error desconocido"}</p>
              </div>
            </div>

            <div className="rounded-md bg-concrete/50 p-4 mt-4 space-y-3 text-sm">
              <p className="font-semibold text-ink">Para activar la integración:</p>
              <ol className="list-decimal list-inside space-y-1.5 text-graphite ml-1">
                <li>En Railway → backend → Variables, agrega:
                  <pre className="mt-1 ml-4 p-2 bg-white rounded text-xs font-mono">
...
ADDI_TRANSACTIONS_PATH=/v1/transactions`}
              ...
              {statusQ.data?.base_url && (
                <div className="text-[0.65rem] text-graphite pt-2 border-t border-border">
                  <span className="font-semibold">Endpoints probados:</span><br/>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-semibold text-teal">Conectado con Addi API</span>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
        <KpiCard label="Transacciones" value={data.total}                            meta={`Últimos ${dias} días`} accent="navy" />
        <KpiCard label="Valor total"   value={formatMoneyShort(data.valor_total)}    meta="Aprobado por Addi"     accent="steel" />
        <KpiCard label="Ticket promedio" value={formatMoneyShort(data.total > 0 ? data.valor_total / data.total : 0)} meta="Por transacción" accent="teal" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[280px]">
          ...
          <input
            ...
            placeholder="Buscar por cliente, email, Addi ID, referencia..."
      ...
      <p className="text-xs text-graphite">Mostrando {filtered.length} de {data.total}</p>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.slice(0, 500).map((t) => (
                    <tr key={t.addi_id} className="border-b border-border hover:bg-concrete/30">
                      <td className="px-3 py-2 text-xs tabular-nums">{t.fecha}</td>
                      <td className="px-3 py-2 font-semibold text-ink">{t.nombre_cliente || "—"}</td>
                      ...
                      <td className="px-3 py-2 text-right tabular-nums font-semibold">{formatMoney(t.valor_bruto)}</td>
                    </tr>
                  ))
                )}
              </tbody>
