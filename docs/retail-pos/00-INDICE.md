...
| H8 | JWT HS256 con **sesión deslizante**, sin refresh token. Revalida contra BD cada 30 s. | `backend/core/security.py:76-110` | Se conserva tal cual. El POS **no tiene login propio**: se entra con correo y contraseña por el ERP y el turno se abre para ese usuario. Ver ADR-006 (revisado). |
